...
ccm.component({
    ...
    Instance: function () {
        ...
        this.init = function (callback) {
            ...
            self.relativeDimensionAndPositionToParent = ccm.helper.clone(self.relativeDimensionAndPositionToParent);
         ...
         */
        this.render = function (callback) { // this function gets called by ccm.js if you call ccm.render(<componentURL>)


            // get the current dom element
            //noinspection JSCheckFunctionSignatures
            ccm.helper.element(self).html('');
            selfsDivElement = $("#ccm-" + self.index);
            ...
            //noinspection JSCheckFunctionSignatures
            selfsDivElement.append(ccm.helper.html(componentHeadDivTemplate, {
                id: "ccm-" + self.index + "-head_div",
                value: ""
            }));

            // set the reference to my Head Div Element
            selfsHeadDivElement = selfsDivElement.find("#ccm-" + self.index + "-head_div");
            ...
            if (!selfsChildAreaDivElement.is('.ui-droppable'))
                $(selfsChildAreaDivElement).droppable({
                    greedy: true,
                ...
                $.contextMenu({
                    ...
                    build: function () { // every time when a context menu ist shown its menu items get `build`
                        ...
                        self.getNodeIsSharedWithItemsForContextMenu(function (sharedCollaboratorsItems) {
                            $("#nodeSharedWith_txtField").append(sharedCollaboratorsItems); // async call will append the value to the text field
                        });
        ...
        this.renderLeafComponent = function (leafComponentURL, loadedFromDatabase, callFromLeafWrapperCreatedWithContextMenu) {
            ...
            selfsLeafAreaDivElement = $("#" + self.index + "-leafArea");
        ...
        this.drawSelfsExistingOutgoingIncomingConnectionsForAlreadyRenderedAdjacentNodes = function () {
            ...
            for (var i = 0; i < adjacentNodesOfSelfArray.length; i++) {
                ...
                var adjacentDivIsRendered = $("#" + adjacentInstance.index + "-child_area_div_element").is("div");
        ...
        this.renderAndScaleDirectChildrenOfZoomedNode = function (nodeChildObjectReferenceArray) {
            ...
            for (var i = 0; i < length; i++) {
                ...
                nodeChildObjectReferenceArray[i].element = $("#" + savedSelfsChildAreaDivElementID);
        ...
        this.logoutButtonPressed = function () {
            ...
            $("#loggedInUserDiv").remove();

            //now a new user can login again
            $("#authorizeButton")[0].disabled = false;


            $("#logoutButton")[0].disabled = true;
        ...
        this.letRootInstanceDoConvergenceLogin = function () {
            ...
            Convergence.connect(self.convergenceDomainUrl, $("#userTextInput").val(), $("#passwordTextInput").val())
        ...
        this.letRootInstanceInitConvergenceApp = function (domain) {
            ...
            $("#logoutButton")[0].disabled = false;


            rootsConvergenceDomain = domain;
            rootsConvergenceModelService = domain.models();

            $("#createNewRealtimeDocButton")[0].disabled = false;
            $("#authorizeButton")[0].disabled = true;
            $("#collaborativeObjectIDButton")[0].disabled = false;
                ...
                selfsModelPermissionManager.getAllUserPermissions() // TODOCONVERGENCE replace getAllUserPermissions with local copy that gets kept  	up-to-date
                    .then(function (allUsersPermissions) {
                            ...
                            for (var [key, value] of allUsersPermissions) {
                                ...
                                //console.log(key);
                                sharedCollaboratorsListItems += '<li style="white-space: nowrap;">' + key + "->" + readWrite + '</li>';
                                i++;
                            }
